...
export async function POST() {
  ...
  if (process.env.NODE_ENV === "development" || true) {
    // Condition to easily enable/disable
    console.warn(
      "TEMPORARY: reCAPTCHA verification is bypassed. API will always return true."
    );
    return NextResponse.json({
      success: true,
      message: "TEMPORARY: reCAPTCHA verification bypassed.",
      score: 1.0, // Simulate a perfect score
    });
  }
  // END TEMPORARY
  ...
  //     console.error(
  //       "Failed to fetch reCAPTCHA verification API:",
  //       verificationResponse.statusText
  //     );
  //     return NextResponse.json(
  //       { success: false, message: "Failed to verify reCAPTCHA with Google." },
  //       { status: 500 }
  //     );
  //   }
  ...
  //     verificationData.score < 0.6
  //   ) {
  //     console.log(
  //       `reCAPTCHA score too low: ${verificationData.score}. Rejecting.`
  //     );
  //     return NextResponse.json(
  //       {
  //         success: false,
  //         message: `Your activity score (${verificationData.score.toFixed(
  //           2
  //         )}) is below the required threshold (0.6). Please try again.`,
  //         score: verificationData.score,
  //       },
  //       { status: 403 }
  //     ); // Forbidden due to low score
  //   }
  ...
  //     successMessage =
  //       "reCAPTCHA verification did not succeed, but proceeding as per rules.";
  //     console.warn(
  //       "reCAPTCHA verification failed with Google, but allowing user through:",
  //       verificationData["error-codes"]
  //     );
  //   } else if (typeof verificationData.score === "number") {
  //     successMessage = `reCAPTCHA score (${verificationData.score.toFixed(
  //       2
  //     )}) is acceptable.`;
  //   } else {
  //     successMessage =
  //       "reCAPTCHA score not available, proceeding as per rules.";
  //   }
  ...
  //   let errorMessage =
  //     "An unknown error occurred during the reCAPTCHA verification process.";
